...
//                做一些数据统计工作
                ...
                User user = new User();
                user.setQq(openID);
                User rs = userService.findUserByOpenId(user);
                if (rs != null) {
                    request.getSession().setAttribute("user", rs);
                    out.print("<script>alert('登录欢迎回来，" + userInfoBean.getNickname() + "');" +
                            "window.location.href='/index.html'</script>");
                } else {
                    User u = new User();
                    u.setQq(openID);
                    u.setUsername(userInfoBean.getNickname());
                    int saveRs = userService.saveUser(u);
                    if (saveRs > 0) {
                        request.getSession().setAttribute("user", u);
                        out.print("<script>alert('你好，" + userInfoBean.getNickname() + "');" +
                                "window.location.href='/index.html'</script>");
                    }else{
